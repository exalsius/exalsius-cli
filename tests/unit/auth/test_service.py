...
class TestAuth0Service:
    @pytest.fixture
    def mock_app_config(self) -> AppConfig:
    ...
    @pytest.fixture
    def auth_service(self, mock_app_config: AppConfig) -> Auth0Service:
        return Auth0Service(config=mock_app_config)

    def test_fetch_device_code(self, auth_service: Auth0Service, mocker: MockerFixture):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
        ...
        result: Auth0DeviceCodeAuthenticationDTO = auth_service.fetch_device_code()

        mock_execute.assert_called_once()
        command: Auth0FetchDeviceCodeCommand = mock_execute.call_args[0][0]
        assert isinstance(command, Auth0FetchDeviceCodeCommand)
        assert result == expected_dto

    def test_fetch_device_code_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
    ...
    def test_poll_for_authentication(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
        expected_dto: Auth0AuthenticationDTO = Auth0AuthenticationDTO(
        ...
        result: Auth0AuthenticationDTO = auth_service.poll_for_authentication(
            "test_device_code"
        )

        mock_execute.assert_called_once()
        command: Auth0PollForAuthenticationCommand = mock_execute.call_args[0][0]
        assert isinstance(command, Auth0PollForAuthenticationCommand)
        assert command.request.device_code == "test_device_code"
        assert result == expected_dto

    def test_poll_for_authentication_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
    ...
    def test_validate_token(self, auth_service: Auth0Service, mocker: MockerFixture):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
        expected_dto: Auth0UserInfoDTO = Auth0UserInfoDTO(
            sub="user_id", email="test@test.com"
        )
        mock_execute.return_value = expected_dto

        result: Auth0UserInfoDTO = auth_service.validate_token("test_id_token")

        mock_execute.assert_called_once()
        command: Auth0ValidateTokenCommand = mock_execute.call_args[0][0]
    ...
    def test_store_token_on_keyring(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
        ...
        mock_execute.assert_called_once()
        command: StoreTokenOnKeyringCommand = mock_execute.call_args[0][0]
        assert isinstance(command, StoreTokenOnKeyringCommand)
        assert command.request.access_token == "test_token"

    def test_store_token_on_keyring_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
    ...
    def test_load_access_token_from_keyring(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
        expected_dto: LoadedTokenDTO = LoadedTokenDTO(
        ...
        result: LoadedTokenDTO = auth_service.load_access_token_from_keyring()

        mock_execute.assert_called_once()
        command: LoadTokenFromKeyringCommand = mock_execute.call_args[0][0]
        assert isinstance(command, LoadTokenFromKeyringCommand)
        assert result == expected_dto

    def test_load_access_token_from_keyring_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
        mock_execute.side_effect = ServiceError("Keyring error")
    ...
    def test_refresh_access_token(
        ...
        result: Auth0AuthenticationDTO = auth_service.refresh_access_token(
            "test_refresh_token"
        )

        mock_execute.assert_called_once()
        command: Auth0RefreshTokenCommand = mock_execute.call_args[0][0]
        assert isinstance(command, Auth0RefreshTokenCommand)
        assert command.request.refresh_token == "test_refresh_token"
        assert result == expected_dto

    def test_refresh_access_token_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
    ...
    def test_acquire_access_token_valid_token(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_load: MagicMock = mocker.patch.object(
            auth_service, "load_access_token_from_keyring"
        )
        mock_refresh: MagicMock = mocker.patch.object(
            auth_service, "refresh_access_token"
        )

        future_expiry = datetime.now() + timedelta(minutes=10)
        loaded_token: LoadedTokenDTO = LoadedTokenDTO(
        ...
        token: str = auth_service.acquire_access_token()
    ...
    def test_acquire_access_token_expired_token_refresh_success(
        ...
        refreshed_auth_dto: Auth0AuthenticationDTO = Auth0AuthenticationDTO(
            access_token="new_access_token",
            id_token="new_id_token",
    ...
    def test_acquire_access_token_expired_token_store_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_load: MagicMock = mocker.patch.object(
            auth_service, "load_access_token_from_keyring"
        ...
        mock_store: MagicMock = mocker.patch.object(
    ...
    def test_acquire_access_token_load_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_load: MagicMock = mocker.patch.object(
            auth_service, "load_access_token_from_keyring"
    ...
    def test_logout_success(self, auth_service: Auth0Service, mocker: MockerFixture):
        mock_load: MagicMock = mocker.patch.object(
            auth_service, "load_access_token_from_keyring"
        )
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
    ...
    def test_logout_revoke_fails(
        ...
        assert mock_execute.call_count == 2
        clear_command: ClearTokenFromKeyringCommand = mock_execute.call_args_list[1][0][
            0
        ]
        assert isinstance(clear_command, ClearTokenFromKeyringCommand)

    def test_logout_clear_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_load: MagicMock = mocker.patch.object(
            auth_service, "load_access_token_from_keyring"
    ...
    def test_logout_revoke_and_clear_fail(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_load: MagicMock = mocker.patch.object(
            auth_service, "load_access_token_from_keyring"
        ...
        assert mock_execute.call_count == 2
        revoke_command: Auth0RevokeTokenCommand = mock_execute.call_args_list[0][0][0]
        clear_command: ClearTokenFromKeyringCommand = mock_execute.call_args_list[1][0][
            0
        ]
        assert isinstance(revoke_command, Auth0RevokeTokenCommand)
    ...
    def test_open_browser_for_device_code_authentication_silent(
        ...
        mock_open_browser: MagicMock = mocker.patch.object(
            auth_service, "_Auth0Service__open_browser"
        )
        mock_webbrowser_get: MagicMock = mocker.patch("webbrowser.get")
    ...
    def test_open_browser_for_device_code_authentication_fails(
        ...
        result: bool = auth_service.open_browser_for_device_code_authentication(
            "http://test.com"
        )
    ...
    def test_reauthorize_with_scope(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
        expected_dto: Auth0AuthenticationDTO = Auth0AuthenticationDTO(
        ...
        _: Auth0AuthenticationDTO = auth_service.reauthorize_with_scope(
            "test_refresh_token", ["openid", "profile", "node:agent"]
        )

        mock_execute_2: MagicMock = mocker.patch.object(
            auth_service, "_execute_command"
        )
        expected_dto_2: Auth0AuthenticationDTO = Auth0AuthenticationDTO(
            access_token="new_access_token",
            id_token="new_id_token",
            scope="openid profile node:agent",
            expires_in=3600,
        ...
        mock_execute_2.return_value = expected_dto_2

        result_2: Auth0AuthenticationDTO = auth_service.reauthorize_with_scope(
            "test_refresh_token", ["openid", "profile", "node:agent"]
        )

        mock_execute_2.assert_called_once()
        command: Auth0RefreshTokenCommand = mock_execute_2.call_args[0][0]
        assert isinstance(command, Auth0RefreshTokenCommand)
        assert command.request.refresh_token == "test_refresh_token"
        assert command.request.scope == ["openid", "profile", "node:agent"]
        assert result_2 == expected_dto_2

    def test_reauthorize_with_scope_fails(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_execute: MagicMock = mocker.patch.object(auth_service, "_execute_command")
    ...
    def test_scope_escalation_check_with_additional_scope(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_reauthorize: MagicMock = mocker.patch.object(
            auth_service, "reauthorize_with_scope"
        )
        ...
        mock_reauthorize.return_value = Auth0AuthenticationDTO(
            access_token="access_token",
            id_token="id_token",
            scope="profile openid node:agent",
            expires_in=3600,
            token_type="Bearer",
        )

        current_scope: List[str] = ["openid", "profile", "node:agent"]
        reference_scope: List[str] = ["openid", "profile", "email"]

        # Should not raise an exception since its essentially the same scope, just different order
    ...
    def test_scope_escalation_check_no_additional_scope(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_reauthorize: MagicMock = mocker.patch.object(
            auth_service, "reauthorize_with_scope"
        ...
        current_scope: List[str] = ["openid", "profile", "email"]
        reference_scope: List[str] = ["openid", "profile"]
    ...
    def test_scope_escalation_check_unexpected_success(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_reauthorize: MagicMock = mocker.patch.object(
            auth_service, "reauthorize_with_scope"
    ...
    def test_scope_escalation_check_wrong_error(
        self, auth_service: Auth0Service, mocker: MockerFixture
    ):
        mock_reauthorize: MagicMock = mocker.patch.object(
            auth_service, "reauthorize_with_scope"
        ...
        mock_reauthorize.return_value = Auth0AuthenticationDTO(
            access_token="access_token",
            id_token="id_token",
            scope="profile openid node:agent email",
            expires_in=3600,
            token_type="Bearer",
        )
        ...
        # Should raise AssertionError since we expect the scope to not change
        with pytest.raises(AssertionError):
            auth_service.scope_escalation_check(
                refresh_token="test_refresh_token",
                current_scope=current_scope,
