...
from exalsius.clusters.models import ClusterType, GPUType, ClusterConfigNodeDTO
...
class ClusterInteractiveFlow:
    ...
    def run(self) -> Optional[str]:
        ...
        self.display_manager.display_welcome("🚀 Cluster Creation - Interactive Mode", "This will guide you through creating a new cluster.\n")
    ...
    def _prompt_cluster_config(self) -> None:
        ...
        self.config.name = questionary.text(
            "Cluster name:",
            default=utils.generate_random_name(prefix="exls-cluster")
        ).ask()
        
        self.config.cluster_type = ClusterType(questionary.select(
            "Cluster type:",
            choices=[
                ...
                questionary.Choice("Adopted (existing k8s cluster)", ClusterType.ADOPTED),
            ],
            default=ClusterType.REMOTE
        ).ask())
        
        self.config.gpu_type = GPUType(questionary.select(
            "GPU type:",
            choices=[
        ...
        self.config.diloco_enabled = questionary.confirm(
            "Enable Diloco/Volcano workload support?",
            default=False
        ).ask()
    ...
    def _prompt_node_selection(self) -> None:
        ...
        selected = questionary.checkbox(
            "Select nodes to add (Space to select, Enter to confirm):",
            choices=choices
        ).ask()
    ...
    def _display_summary(self) -> bool:
        ...
        return questionary.confirm(
            "Create cluster with these settings?",
            default=True
        ).ask()
    ...
    def _create_cluster(self) -> str:
        ...
        control_plane_ids = [
            node_id for node_id, role in self.config.node_roles.items()
        ...
        self.display_manager.display_success(f"Cluster {cluster_id} created successfully.")
        return cluster_id
    ...
    def _prompt_deployment(self, cluster_id: str) -> None:
        ...
        should_deploy = questionary.confirm(
            "Would you like to deploy the cluster now?",
            default=True
        ).ask()
        ...
        if should_deploy:
            try:
                ...
                self.display_manager.display_success(f"Cluster {cluster_id} deployed successfully!")
            except ServiceError as e:
                self.display_manager.display_error(
                    ErrorDTO(
                        message=e.message,
    ...
    def _save_config_to_file(self, cluster_id: str) -> None:
        ...
        if self.config.node_ids:
            config_data["nodes"] = [
                ClusterConfigNodeDTO(
                    node_id=node_id,
                    role=self.config.node_roles.get(node_id, "WORKER")
                ).model_dump()
                for node_id in self.config.node_ids
            ]

        safe_name = "".join(c if c.isalnum() or c in "-_" else "-" for c in self.config.name)
        ...
        try:
            with open(filepath, "w") as f:
                f.write(f"# Cluster configuration generated by exalsius CLI\n")
