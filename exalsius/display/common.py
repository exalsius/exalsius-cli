...
def display_rich_table(df: pd.DataFrame, title: str, id_column: bool = False) -> Table:
    """
    Display a rich table from a pandas DataFrame.

    Args:
        df (pd.DataFrame): The data to display
        title (str): The title for the table
        id_column (bool): Whether to include an ID column

    Returns:
        Table: A rich table object ready for display
    """
    table = Table(
        title=title,
        show_header=True,
